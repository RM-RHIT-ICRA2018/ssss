...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, shared_CNN,optimizer, make_obs_map_ph_n, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i in range(len(obs_ph_n)):
            q_input=tf.concat([q_input,shared_CNN(obs_map_ph_n[i])],1)
        if local_q_func:
        ...
        act = U.function(inputs=[tf.concat([obs_ph_n[p_index],obs_map_ph_n[p_index]],-1)], outputs=act_sample)
        p_values = U.function([tf.concat([obs_ph_n[p_index],obs_map_ph_n[p_index]],-1)], p)
...
class MADDPGAgentTrainer(AgentTrainer):
    def __init__(self, name, model, CNN_model,obs_shape_n, obs_shape_map, act_space_n, agent_index, args, local_q_func=False):
        self.name = name
        ...
        for i in range(self.n):
            obs_ph_n.append(U.BatchInput(obs_shape_n, name="observation"+str(i)).get())
    ...
    def update(self, agents, t):
        ...
        for i in range(self.n):
            ...
            obs_n.append(obs[0])
            obs_next_n.append(obs_next[0])
            obs_map_n.append(obs[1])
            obs_next_map.append(obs_next[1])
            act_n.append(act)
