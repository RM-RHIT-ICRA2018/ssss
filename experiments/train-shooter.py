...
def CNN_model(input, num_outputs=12, scope="CNN", reuse=True):
    # This model takes as input an observation and returns values of all actions
    with tf.variable_scope(scope, reuse=reuse):
              """Model function for CNN."""
        # Input Layer
        input_layer = tf.reshape(input,[1, 53, 83, 1])
...
def get_trainers(env, num_adversaries, obs_shape_n, obs_map_shape, arglist):
    trainers = []
    model = mlp_model
    CNN_model=CNN_model
    ...
    for i in range(num_adversaries):
        trainers.append(trainer(
            "agent_%d" % i, model, CNN_model,obs_shape_n, obs_map_shape, env.action_space, i, arglist,
...
def train(arglist):
    with U.make_session(8):
        ...
        obs_shape_n = [env.obs_dim for i in range(env.n)]
        obs_map_shape=56*86
        num_adversaries = min(env.n, arglist.num_adversaries)
        trainers = get_trainers(env, num_adversaries, obs_shape_n, obs_map_shape, arglist)
